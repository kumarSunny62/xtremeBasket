...
if(strcasecmp($_SERVER['REQUEST_METHOD'], 'GET') == 0){
	...
	*/
	...
	$hash=hash('sha512', $key.'|'.$_GET['txnid'].'|'.$_GET['amount'].'|'.$_GET['productinfo'].'|'.$_GET['firstname'].'|'.$_GET['email'].'|||||'.$_GET['udf5'].'||||||'.$salt);
	...
	$html = '<form action="'.$action.'" id="payment_form_submit" method="post">
			<input type="hidden" id="udf5" name="udf5" value="'.$_GET['udf5'].'" />
			...
			<input type="hidden" id="txnid" name="txnid" value="'.$_GET['txnid'].'" />
			<input type="hidden" id="amount" name="amount" value="'.$_GET['amount'].'" />
			<input type="hidden" id="productinfo" name="productinfo" value="'.$_GET['productinfo'].'" />
			<input type="hidden" id="firstname" name="firstname" value="'.$_GET['firstname'].'" />
			<input type="hidden" id="Lastname" name="Lastname" value="'.$_GET['Lastname'].'" />
			
			<input type="hidden" id="email" name="email" value="'.$_GET['email'].'" />
			<input type="hidden" id="phone" name="phone" value="'.$_GET['phone'].'" />
			
	
			<input type="hidden" id="Pg" name="Pg" value="'.$_GET['UPI'].'" />
...
<style type="text/css">
	 ...
	 body {
            ...
            background: url('Images/bbb.jpg'); /* Default background image for larger screens */
            background-repeat:repeat;
            background-size: cover; /* Make the default image fit small screens */
        }

	/* style sheet from chat gpt  */
	/* style sheet from chat gpt ends here  */
	.main {
		margin-left:30px;
...
<body>
	<div class="main">
		...
		<span style="float:left; display:inline-block;">
			...
			<form action="" id="payment_form" method="get">
			...
			<div class="dv">
				...
				<input type="text" id="txnid" name="txnid" placeholder="Transaction ID"  value="<?php echo  "Txn" . rand(10000,99999999)?>" readonly/></span>
			</div>
			...
			<div class="dv">
				...
				<input type="text" id="amount" name="amount"  placeholder="Amount" value="<?php echo $_POST['btntotal']?>" readonly/></span>    
			</div>
    
			<div class="dv">
				<span class="text"><label>Character ID :</label></span>
				<span>
				<!-- Required - Purchased product/item description or SKUs for future reference. 
				Datatype is Varchar with 100 char limit. //-->
				<input type="text" id="productinfo" name="productinfo" placeholder="Character ID" value="" /></span>
			</div>
			...
			<div class="dv">
				...
				<input type="text" id="phone" name="phone" placeholder="Contact Number" value="" /></span>
			</div>
			...
			<div class="dv">
				...
				<input type="hidden" id="UPI" name="UPI" placeholder="UPI" value="UPI" /></span>
